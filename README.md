...
provider와 standalone을 제외한 core만 사용하려면 `jsr:@yoonsung/rlm/core` subpath를 사용할 수
있습니다.
...
browser와 일반 Node ESM 소비를 위해 core 전용 ESM bundle을 만들 수 있습니다.
...
즉 `dist/core/index.mjs`는 browser-safe core 배포물이고, provider와 standalone은 full package/source
경로에서 계속 사용합니다.
...
`createOpenAIRLM(...)`과 `runOpenAIRLM(...)`은 이 일반 구조 위에 얹혀 있는 OpenAI 편의 진입점입니다.
구현과 공개 import 경로는 provider 모듈에 있습니다.
...
이 둘은 `createRLM(...)`이 요구하는 일반 인터페이스를 따르는 구체 provider입니다.

- `OpenAIResponsesProvider` import:
  - `jsr:@yoonsung/rlm/providers/openai`
- `CodexOAuthProvider` import:
  - `jsr:@yoonsung/rlm/providers/codex-oauth`

core build 산출물인 `dist/core/index.mjs`에는 포함되지 않습니다.
...
진입점입니다. provider 전용 import를 쓰려면 `jsr:@yoonsung/rlm/providers/openai`, 기존 호환 경로를
쓰려면 `jsr:@yoonsung/rlm`에서 가져오면 됩니다.
...
이 저장소에는 다음 provider 구현이 포함되어 있습니다. provider는 root package가 아니라 provider
subpath에서 import하는 것을 기준으로 봐야 합니다.
...
browser에서는 provider를 직접 import하지 않아도 됩니다. core는 `LLMCaller`만 알면 되므로, browser는
자신에게 맞는 호출 경로를 `LLMCaller`로 감싸서 주입하면 됩니다.

### 1. 브라우저 -> 자체 백엔드 프록시

가장 일반적인 방식입니다. browser는 provider 인증을 직접 다루지 않고, 서버가 실제 provider를
호출합니다.

```ts
import type { LLMCaller } from 'jsr:@yoonsung/rlm/core';
import { createRLM } from 'jsr:@yoonsung/rlm/core';

const llm: LLMCaller = {
  async complete(request) {
    const response = await fetch('/api/rlm/complete', {
      method: 'POST',
...
### 2. 브라우저 -> browser-safe provider

브라우저에서 직접 사용할 수 있는 session token, ephemeral token, 내부 gateway SDK가 있다면
browser-safe provider를 직접 구현할 수 있습니다.

```ts
import type { LLMCaller, LLMProvider } from 'jsr:@yoonsung/rlm/core';

class BrowserGatewayProvider implements LLMProvider<{ token: string }> {
  createCaller(config: { token: string }): LLMCaller {
    return {
      async complete(request) {
        const response = await fetch('https://gateway.example.com/complete', {
          method: 'POST',
          headers: {
...
### 3. 브라우저 -> WebSocket / worker bridge

장기 세션, streaming, 서버 측 continuation state를 유지해야 하면 browser는 WebSocket이나 dedicated
worker bridge를 통해 `LLMCaller`를 구현할 수 있습니다.

```ts
import type { LLMCaller } from 'jsr:@yoonsung/rlm/core';

function createSocketCaller(socket: WebSocket): LLMCaller {
  let nextId = 0;
  ...
  return {
    async complete(request) {
      ...
      const response = await new Promise((resolve, reject) => {
        ...
        setTimeout(() => reject(new Error('Socket completion timed out.')), 30_000);
      });

      return response as Awaited<ReturnType<LLMCaller['complete']>>;
    },
  };
}
```

위 세 방식 모두 core가 보는 인터페이스는 동일합니다.

- 입력: `LLMCallerRequest`
- 출력: `LLMCallerResponse`
- 주입 위치: `createRLM({ llm, models })`

### one-shot helper

객체를 만들지 않고 한 번만 실행하고 싶다면 `runOpenAIRLM(...)` 또는 `runRLM(...)`을 사용할 수
있습니다. `runOpenAIRLM(...)`은 provider subpath에서 import합니다.

```ts
import { NullRLMLogger } from 'jsr:@yoonsung/rlm';
import { runOpenAIRLM } from 'jsr:@yoonsung/rlm/providers/openai';
...
```ts
import { InMemoryRLMLogger } from 'jsr:@yoonsung/rlm';
import { createOpenAIRLM } from 'jsr:@yoonsung/rlm/providers/openai';
...
```ts
import { JsonlFileLogger } from 'jsr:@yoonsung/rlm';
import { createOpenAIRLM } from 'jsr:@yoonsung/rlm/providers/openai';
...
```ts
import { WorkerExecutionBackend } from 'jsr:@yoonsung/rlm';
import { createOpenAIRLM } from 'jsr:@yoonsung/rlm/providers/openai';
...
standalone는 full package 기능이며, `dist/core/index.mjs` build에는 포함되지 않습니다. 현재
...
실제 Codex OAuth 통합 테스트:

```bash
deno task test:codex
deno task test:codex:integration
```

`deno task test:codex`는 catalog 모델에 대한 `ping -> PONG` smoke를 수행합니다. 통합 시나리오
스위트는 단일 진입점 `tests/openai_live_test.ts`를 사용하고, 현재는 Codex OAuth provider를 통해
실행됩니다.
...
```bash
deno task test:codex:integration
deno task test:codex:synthetic
```

long-context 벤치:

```bash
deno task test:codex:long-context
```
...
provider 전용 진입점은 다음 subpath로도 사용할 수 있습니다.
